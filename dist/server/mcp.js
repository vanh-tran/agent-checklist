...
function broadcastAgent(b, agent) {
    const msg = { type: "agent_updated", payload: structuredClone(agent) };
    b.broadcast(msg);
}
function broadcastTask(b, agentId, task) {
    const msg = { type: "task_updated", payload: { agentId, task: structuredClone(task) } };
    b.broadcast(msg);
}
export function createMcpToolHandlers(ctx) {
    return {
...
export function createMcpServer(ctx) {
    const mcp = new McpServer({ name: "agent-checklist", version: "0.1.0" }, { instructions: "Publish the calling agent's task checklist and real-time progress to the local dashboard." });
    const h = createMcpToolHandlers(ctx);
    mcp.tool("register_agent", "Register the calling agent with its planned task checklist. Use $CLAUDE_SESSION_ID as agentId when available. Safe to call again with the same agentId and name (re-registers without duplicating tasks).", {
    ...
    mcp.tool("update_task", "Set a task's status. Setting one task to in_progress auto-reverts any other in_progress task on the same agent to pending.", {
    ...
    mcp.tool("add_tasks", "Append new tasks to an existing agent. Provide afterTaskId to insert in-place instead of appending.", {
        agentId: z.string().min(1),
        tasks: z.array(z.string().min(1)),
        afterTaskId: z.string().min(1).optional(),
    }, h.add_tasks);
    mcp.tool("remove_task", "Remove a task. Refuses tasks that are in_progress; set them to pending or completed first.", { agentId: z.string().min(1), taskId: z.string().min(1) }, h.remove_task);
    mcp.tool("reorder_tasks", "Replace the ordered task list. taskIds must contain exactly the agent's current task IDs.", { agentId: z.string().min(1), taskIds: z.array(z.string().min(1)) }, h.reorder_tasks);
    mcp.tool("rename_task", "Update a task's label. Status and other fields are unchanged.", { agentId: z.string().min(1), taskId: z.string().min(1), label: z.string().min(1) }, h.rename_task);
    mcp.tool("get_board", "Return the full current board state (all agents and tasks).", {}, h.get_board);
    return mcp;
}
