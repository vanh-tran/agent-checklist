...
async function probeReady(port, timeoutMs = 1500) {
    const ctrl = new AbortController();
    const t = setTimeout(() => ctrl.abort(), timeoutMs);
    try {
...
async function waitForReady(port, overallMs = 5000) {
    const until = Date.now() + overallMs;
    while (Date.now() < until) {
        if (await probeReady(port, 500))
            return true;
...
export function buildCli() {
    ...
    program
        ...
        try {
            ...
            console.error(`failed to signal pid ${h.pid}: ${err.message}`);
            process.exit(1);
        }
